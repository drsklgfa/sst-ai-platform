...
O Web usa o `ENTRYPOINT` e o `CMD` do Dockerfile. Na primeira adoção da baseline use `DB_SCHEMA_MODE=bootstrap`; depois use `DB_SCHEMA_MODE=migrate`.
...
No Worker, defina `DB_SCHEMA_MODE=none` para impedir qualquer alteração do schema.
...
1. Abra `https://SEU-DOMINIO/api/health/live` e depois `https://SEU-DOMINIO/api/health`.
