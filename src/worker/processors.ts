...
import { analyzeLegacyImportDocument } from '@/lib/legacy-imports';
import { analyzeFieldCapture } from '@/lib/field-operations';
import { processPaymentWebhookEvent } from '@/lib/integrations/payments';
import { transmitQueuedEsocialEvent } from '@/lib/esocial';
...
export async function processJob(job: { type: string; payload: any; tenantId: string }) {
  switch (job.type) {
    ...
    case 'LEGACY_ANALYZE_DOCUMENT':
      return analyzeLegacyImportDocument(job.payload.documentId);
    case 'FIELD_ANALYZE_CAPTURE':
      return analyzeFieldCapture(job.payload.captureId);
    case 'PAYMENT_WEBHOOK_PROCESS':
      return processPaymentWebhookEvent({ eventId: job.payload.eventId, resourceId: job.payload.resourceId });
    case 'ESOCIAL_TRANSMIT':
      return transmitQueuedEsocialEvent({ tenantId: job.tenantId, eventId: job.payload.eventId });
    case 'RETENTION_RUN':
