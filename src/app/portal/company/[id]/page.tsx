...
export default async function CompanyPortal({ params }: { params: Promise<{ id: string }> }) {
  ...
  const canPortalRequests = env.FEATURE_CLIENT_PORTAL_PLUS && hasCompanyPermission(access.role, 'portal.request.read', access.permissions);
  const canCreatePortalRequests = env.FEATURE_CLIENT_PORTAL_PLUS && hasCompanyPermission(access.role, 'portal.request.create', access.permissions);

  const [campaigns, documents, plans, conversationCount, unreadNotifications, portalRequests] = await Promise.all([
    canDashboard
    ...
    canPortalRequests ? db.companyPortalRequest.findMany({ where: { companyId: id }, orderBy: { createdAt: 'desc' }, take: 30 }) : Promise.resolve([]),
  ]);
  ...
  return (
    <main className="shell min-h-screen p-6"><div className="mx-auto max-w-6xl">
      ...
      {canPortalRequests && <Card className="mt-6"><div className="flex items-center justify-between"><div><h2 className="font-bold">Solicitações à consultoria</h2><p className="text-sm text-slate-500">Envie pedidos de documento, atualização, visita, treinamento ou suporte.</p></div><Badge>{portalRequests.length}</Badge></div>{canCreatePortalRequests && <form action={`/api/portal/companies/${id}/requests`} method="post" className="mt-4 grid gap-3"><div className="grid gap-3 md:grid-cols-3"><select name="category" className="rounded-xl border border-slate-300 px-3 py-2.5"><option value="DOCUMENT">Documento</option><option value="UPDATE">Atualização</option><option value="VISIT">Visita</option><option value="TRAINING">Treinamento</option><option value="SUPPORT">Suporte</option></select><select name="priority" className="rounded-xl border border-slate-300 px-3 py-2.5"><option value="LOW">Baixa</option><option value="MODERATE">Normal</option><option value="HIGH">Alta</option><option value="CRITICAL">Crítica</option></select><Input name="dueAt" type="date"/></div><Input name="title" required placeholder="Assunto"/><Textarea name="description" required minLength={10} rows={3} placeholder="Descreva o que precisa"/><Button type="submit">Enviar solicitação</Button></form>}<div className="mt-4 divide-y">{portalRequests.map((request) => <div key={request.id} className="flex flex-wrap items-start justify-between gap-2 py-3"><div><p className="font-medium">{request.title}</p><p className="text-xs text-slate-500">{request.category} · criada em {request.createdAt.toISOString().slice(0,10)}{request.dueAt ? ` · prazo ${request.dueAt.toISOString().slice(0,10)}` : ''}</p><p className="mt-1 text-sm text-slate-600">{request.description}</p></div><Badge tone={request.status === 'CLOSED' ? 'success' : request.priority === 'CRITICAL' ? 'danger' : 'warning'}>{request.status}</Badge></div>)}</div></Card>}
    </div></main>
  );
}
