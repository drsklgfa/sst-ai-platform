...
import { nonNegativeInteger, optionalText, requiredText } from '@/domain/companies/validation';
...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const rawCnpj = String(form.get('cnpj') ?? '').trim();
    const cnpj = rawCnpj ? normalizeCnpj(rawCnpj) : null;
    if (rawCnpj && !cnpj) return new Response('CNPJ deve conter 14 posições válidas; as 12 primeiras podem ser alfanuméricas e as 2 últimas devem ser numéricas', { status: 400 });
