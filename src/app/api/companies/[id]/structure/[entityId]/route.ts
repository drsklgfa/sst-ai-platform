...
export async function POST(request: Request, { params }: { params: Promise<{ id: string; entityId: string }> }) {
  ...
  try {
    ...
    if (entityType === 'establishment') {
      ...
      if (operation === 'update' && rawCnpj && !cnpj) return new Response('CNPJ deve conter 14 posições válidas; as 12 primeiras podem ser alfanuméricas e as 2 últimas devem ser numéricas', { status: 400 });
