...
export async function GET(request: Request) {
  ...
  const [backup, internalMessageAttachment, internalCommentAttachment, aiAttachment, fieldCapture] = await Promise.all([
    ...
    db.aIMessageAttachment.findFirst({ where: { fileObjectId: file.id, aiMessage: { aiThread: { tenantId: file.tenantId } } }, select: { id: true } }),
    db.fieldCapture.findFirst({ where: { fileObjectId: file.id, fieldVisit: { tenantId: file.tenantId } }, select: { id: true } }),
  ]);
  if (membership) {
    authorized = backup
      ...
      : internalMessageAttachment || internalCommentAttachment
        ...
        : aiAttachment
          ? hasTenantPermission(membership.role, 'work.manage', membership.permissions)
          : fieldCapture
            ? hasTenantPermission(membership.role, 'inspection.manage', membership.permissions)
